...
function openColorPicker(context, filePath) {
    ...
    const panel = vscode.window.createWebviewPanel('imageColorPicker', 'Image Color Picker', vscode.ViewColumn.Two, {
        enableScripts: true,
...
async function handleColorPicked(hex, rgb) {
    const action = await vscode.window.showInformationMessage(`Color picked: ${hex}`, 'Copy to Clipboard', 'Save to File', 'Insert at Cursor');
    switch (action) {
...
async function saveColorToFile(hex) {
...
function getWebviewContent(imageUri, fileName) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
